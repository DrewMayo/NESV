...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int ret = XSpi_Transfer(&SpiInstance, send, NULL, nbytes + 1);
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	if (ret != 0) {
		xil_printf("MAXbytes_WR, Error: %X\n", ret);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE send[2] = {reg, 0};
	BYTE recv[2];
	int ret = XSpi_Transfer(&SpiInstance, send, recv, 2);
	if (ret != 0) {
		xil_printf("MAXreg_rd, Error: %X\n", ret);
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0); //deselect
	return recv[1];
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	send[0] = reg;
	int ret = XSpi_Transfer(&SpiInstance, send, recv, nbytes + 1);
	for(int i = 0; i < nbytes; ++i) {
		data[i] = recv[i + 1];
	}
	if (ret != 0) {
		xil_printf("MAXbytes_rd, Error: %X\n", ret);
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0); //deselect
	return (data + nbytes);
}
